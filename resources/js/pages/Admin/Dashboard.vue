        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="grid auto-rows-min gap-4 md:grid-cols-3">
                <div class="relative aspect-video overflow-hidden rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                    <h3 class="space-x-2 font-bold">
                        <Link :href="route('admin.users.index')" class="underline">Users:</Link>
                        <span class="font-normal">{{ stats.usersCount }}</span>
                    </h3>

                    <Chart title="New Users" :labels="trends.users.map((u) => u.date)" :data="trends.users.map((u) => u.count)" class="p-5" />
                </div>

                <div class="relative aspect-video overflow-hidden rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                    <h3 class="space-x-2 font-bold">
                        <Link :href="route('admin.questions.index')" class="underline">Questions:</Link>
                        <span class="font-normal">{{ stats.questionsCount }}</span>
                    </h3>

                    <Chart
                        title="Daily Questions"
                        :labels="trends.questions.map((q) => q.date)"
                ...
                <div class="relative aspect-video overflow-hidden rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                    <h3 class="space-x-2 font-bold">
                        <Link :href="route('admin.answers.index')" class="underline">Answers:</Link>
                        <span class="font-normal">{{ stats.answersCount }}</span>
                    </h3>

                    <Chart title="Daily Answers" :labels="trends.answers.map((a) => a.date)" :data="trends.answers.map((a) => a.count)" class="p-5" />
                </div>
            </div>
            ...
            <div class="grid auto-rows-min gap-4 md:grid-cols-3">
                <div class="relative overflow-hidden rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                    <h3 class="space-x-2 font-bold">
                        <Link :href="route('admin.tags.index')" class="underline">Tags:</Link>
                        <span class="font-normal">{{ stats.tagsCount }}</span>
                    </h3>
                </div>

                <div class="relative overflow-hidden rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                    <h3 class="space-x-2 font-bold">
                        <Link :href="route('admin.comments.index')" class="underline">Comments:</Link>
                        <span class="font-normal">{{ stats.commentsCount }}</span>
                    </h3>
                </div>

                <div class="relative overflow-hidden rounded-xl border border-sidebar-border/70 p-4 dark:border-sidebar-border">
                    <h3 class="font-bold">
                        Votes: <span class="font-normal">{{ stats.votesCount }}</span>
                    </h3>
                </div>
            </div>
                ...
                <DashboardTable
                    ...
                    :rows="
                        latest.questions.map((q) => [
                            ...
                            { text: `@${q.user.username}`, href: route('profile.activities', q.user.username) },
